...
保险库保护 L3 机器秘密和人类秘密，默认拒绝 L4。保险库主口令始终禁止作为记录保存。
支付口令或银行 PIN 只有在所有者完成当前 Passkey step-up、再次确认并提交固定策略覆盖值时，
才可按不可共享的 `human_secret` 保存。它提供最小元数据查询、受控 `use`、增强认证后的
`reveal`、更新、删除、导出、授权和轮换。
...
- L4 所有者覆盖记录只允许所有者访问，不能授予他人、不能 `use`，也不能交给自动化服务。
...
- L4 默认拒绝、错误覆盖值、缺少显式确认、跨用户访问和自动化 `use`；
